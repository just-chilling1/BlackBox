  ...
  return (
    <motion.section variants={itemVariants} className="card-base">
      <div className="mb-5 flex items-center gap-3">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-accent-muted/20 bg-accent-muted/10">
          <FileText className="h-6 w-6 text-accent-muted" />
        </div>
        <div>
      ...
      <div className="space-y-4">
        {refundPolicy.items.map((item) => (
          <div key={item.title} className="rounded-xl border border-border-dim/80 bg-page/60 p-4">
            <h3 className="ds-h4 mb-2 text-accent-readable">{item.title}</h3>
